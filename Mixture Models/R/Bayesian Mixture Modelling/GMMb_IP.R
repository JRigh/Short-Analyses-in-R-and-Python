...
Gibbs.gaussian.mixture.IP <- function (y, k=2, n.keep, n.gap=1, n.burn) {
  ...
  for (iter in 1:n.sim) {
    ...
    ## Sample mixture means (mu) from a normal distribution
    for (j in 1:k) {
      ## Set mixture sample mean
      sum.y <- sum(y[z[,j]==1])
      ## Set posterior mean and standard deviation
      mu.mean <- (m.0[j]/v.0[j] + sum.y/sigma2[j]) / (1/v.0[j] + n.mix[j]/sigma2[j])
      mu.sd <- sqrt(1/( 1/v.0[j] + n.mix[j]/sigma2[j] ))
    ...
    for (j in 1:k) {
      ...
      sigma2.shape <- sh.0[j]  + 0.5*n.mix[j]
      sigma2.rate <- (ra.0[j] + 0.5*sum( z[,j] * (y - mu[j])^2 ))
...
# perform GMM
set.seed(2024) 
gmm = Gibbs.gaussian.mixture.IP(y = faithful$waiting , k = 2, n.keep = 10000, n.burn = 1000)
gmm
